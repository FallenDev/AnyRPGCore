...
namespace AnyRPG {
    ...
    public class TauntEffectConfig : AbilityEffectConfig {
        ...
        public override string Convert(Ability ability, string pathName) {
            ...
            string effectType = "Taunt";
            Debug.Log($"{effectType}EffectConfig.Convert({ability.resourceName})");
            TauntEffect newAbilityEffect = ScriptableObject.CreateInstance($"{effectType}Effect") as TauntEffect;
            newAbilityEffect.tauntEffectProperties = effectProperties;
