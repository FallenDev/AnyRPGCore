    ...
    public class CraftingManager : ConfiguredMonoBehaviour {
        ...
        private bool CanCraft(Recipe recipe) {
            for (int i = 0; i < recipe.CraftingMaterials.Count; i++) {
                if (playerManager.UnitController.CharacterInventoryManager.GetItemCount(recipe.CraftingMaterials[i].Item.ResourceName) < recipe.CraftingMaterials[i].Count) {
                    return false;
                }
            }
            return true;
        ...
        public int GetMaxCraftAmount(Recipe checkRecipe) {
            ...
            for (int i = 0; i < checkRecipe.CraftingMaterials.Count; i++) {
                int possibleAmount = playerManager.UnitController.CharacterInventoryManager.GetItemCount(checkRecipe.CraftingMaterials[i].Item.ResourceName) / checkRecipe.CraftingMaterials[i].Count;
                if (maxAmount == -1) {
        ...
        public void BeginCrafting() {
            ...
            if (currentRecipe != null) {
                ...
                playerManager.UnitController.CharacterAbilityManager.BeginAbility(craftAbility);
            } else {
                //Debug.Log("MySelectedRecipeScript is null!");
            }
        }
        ...
        public void CraftNextItem() {
            ...
            if (GetMaxCraftAmount(craftingQueue[0]) > 0) {
                InstantiatedItem tmpItem = playerManager.UnitController.CharacterInventoryManager.GetNewInstantiatedItem(craftingQueue[0].Output.ResourceName);
        ...
        public void CancelCrafting() {
            ...
            craftingQueue.Clear();
            playerManager.UnitController.CharacterAbilityManager.TryToStopCasting();
        }
