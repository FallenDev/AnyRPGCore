...
namespace AnyRPG {
    ...
    public class TeleportEffectConfig : AbilityEffectConfig {
        ...
        public override string Convert(Ability ability, string pathName) {
            ...
            string effectType = "Teleport";
            Debug.Log($"{effectType}EffectConfig.Convert({ability.resourceName})");
            TeleportEffect newAbilityEffect = ScriptableObject.CreateInstance($"{effectType}Effect") as TeleportEffect;
            newAbilityEffect.teleportEffectProperties = effectProperties;
