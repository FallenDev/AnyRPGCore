...
namespace AnyRPG {
    ...
    public class InstantEffectConfig : AbilityEffectConfig {
        ...
        public override string Convert(Ability ability, string pathName) {
            ...
            string effectType = "Instant";
            Debug.Log($"{effectType}EffectConfig.Convert({ability.resourceName})");
            InstantEffect newAbilityEffect = ScriptableObject.CreateInstance($"{effectType}Effect") as InstantEffect;
            newAbilityEffect.instantEffectProperties = effectProperties;
