...
namespace AnyRPG {
    ...
    public class ChanneledEffectConfig : AbilityEffectConfig {
        ...
        public override string Convert(Ability ability, string pathName) {
            ...
            string effectType = "Channeled";
            Debug.Log($"{effectType}EffectConfig.Convert({ability.resourceName})");
            ChanneledEffect newAbilityEffect = ScriptableObject.CreateInstance($"{effectType}Effect") as ChanneledEffect;
            newAbilityEffect.channeledEffectProperties = effectProperties;
